...
from data import DatasetHelper, ed_DatasetHelper
...
from modeling_gpt2 import KnowledgeTextGen, GPT2Config
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, drop_last=True, num_workers=args.workers, pin_memory=True)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            # first generate kg
            # pred_kg_batch = generate_kg(args, model, generator, batch)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    logger.info("Step: {} | Loss: {:.4f} | Loss: {:.4f}".format(global_step,
...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0'):
    ...
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, drop_last=True, num_workers=args.workers, pin_memory=True)
        ...
        with torch.no_grad():
            if 'bleu' in evaluate_metrics:  # when inference
                model_batch = {"src_input_ids": batch[0],
                               ...
                               "pred_kg": batch[8],
                               "pred_kg_attention_mask": batch[9],
                               "pred_kg_position_ids": batch[10],
                               "greedy_or_beam": "greedy" if args.beam == 1 else "beam"
                               }

                if isinstance(model,torch.nn.DataParallel):
                    tg_hypos, multi_tg_hypos = model.module.autoreg_generate(**model_batch)
                else:
                    tg_hypos, multi_tg_hypos = model.autoreg_generate(**model_batch)
    ...
    if 'bleu' in evaluate_metrics:
        assert len(tgt)==len(tgen_seqs),'prediction is not finished !'
        print_data = {'src': src, 'tgt': tgt, 'prediction': tgen_seqs,
                      'src_kg': kg_src, 'prefix': prefix}
    ...
    if 'dist' in evaluate_metrics:
        predictions = [x.split() for x in tgen_seqs]
...
def generate_kg(args, model, generator, batch):
    model.eval()
    if args.do_eval:
       batch = tuple(t.to(args.device) for t in batch[0])
    else:
    ...
    with torch.no_grad():
        batch = {"seq_generator": generator,
                 "kg_input_ids": batch[8],
                 "kg_input_rel_ids": batch[9],
                 "kg_attention_mask": batch[10],
                 "kg_position_ids": batch[11],
                 "tgt_bos_rel_ids": batch[16],
                 "is_end_at_relation": batch[17],
                 "greedy_or_beam": "greedy" if args.beam == 1 else "beam",}

        if isinstance(model, torch.nn.DataParallel):
            new_batch = model.module.autoreg_generate_kg(**batch)
        else:
            new_batch = model.autoreg_generate_kg(**batch)
...
def save_generation(args, src, tgt, prediction, src_kg, prefix='0'):
...
def main():
    ...
    ## File parameters
    parser.add_argument("--data_file", default=None, type=str, required=True, help="The input training data file (a text file).")
    parser.add_argument("--train_data_file", default="ed_train_prop.json", type=str, required=True, help="The input training data file (a text file).")
    parser.add_argument("--dev_data_file", default="ed_dev_prop.json", type=str, required=True, help="The input dev data file (a text file).")
    parser.add_argument("--test_data_file", default="ed_test_prop.json", type=str, required=True, help="The input test data file (a text file).")
    parser.add_argument("--train_rel_data_file", default="ed_train_prop.json", type=str, help="The input training relation ids data file (a text file).")
    parser.add_argument("--dev_rel_data_file", default="ed_dev_prop.json", type=str, help="The input dev relation ids data file (a text file).")
    parser.add_argument("--test_rel_data_file", default="ed_test_prop.json", type=str, help="The input test relation ids data file (a text file).")
    ...
    parser.add_argument("--kg_model_name_or_path", default="fine-tuned-gpt2", type=str, help="The kg model checkpoint for weights initialization.")
    ...
    if args.do_eval:
        ...
        kg_load_from_path = args.output_dir
    elif args.continue_train:
        load_from_path = args.output_dir
        kg_load_from_path = args.output_dir
        print('continue training from {}'.format(args.output_dir))
    else:
        load_from_path = args.model_name_or_path
        kg_load_from_path = args.kg_model_name_or_path
