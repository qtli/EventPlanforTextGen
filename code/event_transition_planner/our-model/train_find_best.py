...
from data import roc_DatasetHelper, ed_DatasetHelper
    ...
    for epoch in train_iterator:
        ...
        for step, ori_batch in enumerate(train_dataloader):
            ...
            if global_step % args.validate_steps == 0:
                ...
                result = evaluate(args, model, tokenizer, 'ppl', prefix='dev')
                logger.info(
                    "Epoch {} evaluate {}: {:.4f}".format(epoch, 'ppl', result[args.evaluate_metrics]))

                if os.path.exists(os.path.join(args.output_dir, str(global_step))) is False:
                    os.makedirs(os.path.join(args.output_dir, str(global_step)))


                model_to_save = model.module if hasattr(model,
                                                        'module') else model  # Take care of distributed/parallel training
                model_to_save.save_pretrained(os.path.join(args.output_dir, str(global_step)))
                torch.save(args, os.path.join(args.output_dir, str(global_step), 'training_args.bin'))
                torch.save(scheduler.state_dict(), os.path.join(args.output_dir, str(global_step), "scheduler.bin"))
                torch.save(optimizer.state_dict(), os.path.join(args.output_dir, str(global_step), "optimizer.bin"))
                subprocess.call(["cp", os.path.join(args.model_name_or_path, str(global_step), "vocab.json"), args.output_dir])
                subprocess.call(["cp", os.path.join(args.model_name_or_path, str(global_step), "merges.txt"), args.output_dir])
                logger.info("Saving model checkpoint to %s", args.output_dir + '/' + str(global_step) )
                best_valid[args.evaluate_metrics] = result[args.evaluate_metrics]

                # test !!!!!
                print('------------------RUN TEST------------------')
                test_result = evaluate(args, model, tokenizer, 'bleu_dist', prefix=str(global_step))
                print(test_result)
                ...
                print('baseline, bleu1: 8.48; bleu2: 3.35; bleu4: 0.91')
                print('before that, bleu1: 8.50; bleu2: 3.38; bleu4: 0.93 \n whether to continue training')
...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0'):
    ...
    eval_dataset = DH(args,
                      ...
                      do_generate=args.do_eval or 'bleu' in evaluate_metrics,
        ...
        with torch.no_grad():
            if 'bleu' in evaluate_metrics:  # when inference
                model_batch = {"src_input_ids": batch[0],
                               ...
                               "kg_input_ids": batch[8],  # todo
                               "kg_rel_ids": batch[9],  # todo
...
def save_generation(args, src, tgt, prediction, kg_path, prefix='0'):
...
def main():
    ...
    if args.do_eval:
        ...
        if args.beam == 1:
            ...
            args.output_dir = os.path.join(args.output_dir, '_beam_eval')
