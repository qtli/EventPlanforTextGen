...
from data import DatasetHelper, roc_DatasetHelper, ed_DatasetHelper
...
from modeling_gpt2 import KnowledgeGen, GPT2Config, GPT2Model
...
def train(args, train_dataset, model, tokenizer):
    ...
    # total_batch_per_epoch = len(train_dataloader)
    # train_batch_per_epoch = total_batch_per_epoch * 0.5
    # logger.info("  Total batch per epoch = %d", train_batch_per_epoch)
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            batch = [batch[0], batch[1], batch[2], batch[3], batch[4], batch[5], batch[6], batch[7],
                     args.relation_embed, args.fine_tune_step, args.ex_preseqlen, args.task_preseqlen]
            ...
            if (global_step) % args.validate_steps == 0:
                ...
                if (result[args.evaluate_metrics] - best_valid[args.evaluate_metrics]) * sign_list[
                    ...
                    logger.info('Patient is {}.'.format(patient))
                    if patient > 2:
                        logger.info('Patient has reach {} and Stop training.'.format(patient))
                        break
            if patient > 2:
                break
        if patient > 2:
...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0'):
    ...
    if 'atomic' in args.knowledge_type:
        dh = DatasetHelper
    elif 'roc' in args.knowledge_type:
    ...
    total_batch_per_epoch = len(eval_dataloader)
    eval_batch_per_epoch = total_batch_per_epoch * 0.5
    logger.info("  Total batch per epoch = %d", eval_batch_per_epoch)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        if prefix == 'test' or args.do_eval:
            src.extend(batch[1])
            tgt.extend(batch[2])
        ...
        with torch.no_grad():
            if 'bleu' in evaluate_metrics:
                eval_batch = {"src_input_ids": batch[0],
                              "src_input_rel_ids": batch[1],
                              "attention_mask": batch[2],
                              "src_position_ids": batch[3],
                              "seq_generator": generator,
                              "tgt_bos_rel_ids": batch[8],
                              "is_end_at_relation": batch[9],
                              "greedy_or_beam": "greedy" if args.beam == 1 else "beam",
                              "relation_embed": args.relation_embed,
                              "fine_tune_step": args.fine_tune_step,
                              "ex_preseqlen": args.ex_preseqlen,
                              "task_preseqlen": args.task_preseqlen, }
                if isinstance(model, torch.nn.DataParallel):
                    hypos, multi_hypos = model.module.aotoreg_generate(**eval_batch)
                else:
                    hypos, multi_hypos = model.aotoreg_generate(**eval_batch)
                gen_seqs.extend(hypos)
                multi_gen_seqs.extend(multi_hypos)
                if nb_eval_steps == 0:
                    print('prediction sample: ')
                    # print(multi_gen_seqs[0])
                    print(hypos[0])
                    pdb.set_trace()
    ...
    if 'bleu' in evaluate_metrics:
        print_data = {'src': src, 'tgt': tgt, 'prediction': gen_seqs, 'prefix': args.prediction_file_prefix}
        if args.beam > 1:
            print_data['multi_prediction'] = multi_gen_seqs
        save_generation(args, **print_data)
        # assert len(gen_seqs) == len(tgt)

        if args.beam == 1:
            predictions = [x['prediction'].split() for x in gen_seqs]
        else:
        ...
        bleu1 = _compute_bleu(references, predictions, max_order=2)
...
def save_generation(args, src, tgt, prediction, multi_prediction=None, prefix='0'):
    ...
    with open(save_result_dir, 'w') as f:
        for i, line in enumerate(src):
            ...
            if args.beam == 1:
                f.write('text prediction: ' + prediction[i]['prediction'] + '\n')
            else:
                f.write('text prediction: ' + prediction[i].strip() + '\n')

            if multi_prediction is not None:
                f.write('beam text prediction: ' + str(multi_prediction[i]) + '\n')
...
def main():
    ...
    parser.add_argument("--train_data_file", default="train_sim.txt", type=str, required=True,
                        help="The input training data file (a text file).")
    parser.add_argument("--dev_data_file", default="dev_sim.txt", type=str, required=True,
                        help="The input dev data file (a text file).")
    parser.add_argument("--test_data_file", default="test_sim.txt", type=str, required=True,
                        help="The input test data file (a text file).")
    parser.add_argument("--train_rel_data_file", default="train_rel_id.json", type=str,
                        help="The input training relation ids data file (a text file).")
    parser.add_argument("--dev_rel_data_file", default="dev_rel_id.json", type=str,
                        help="The input dev relation ids data file (a text file).")
    parser.add_argument("--test_rel_data_file", default="test_rel_id.json", type=str,
    ...
    parser.add_argument("--fine_tune_step", default=0, type=int, help="fine-tune on external kg or task kg")
    parser.add_argument("--ex_preseqlen", default=5, type=int, help="prefix length for atomic prefix tuning")
    parser.add_argument("--task_preseqlen", default=5, type=int, help="prefix length for task data (roc/ed) prefix tuning")
    parser.add_argument("--preseqdim", default=384, type=int, help="middle dim of prefix embedding layer")
    parser.add_argument("--prefix_dropout", default=0.0, type=float, help="prefix dropout")
    ...
    parser.add_argument("--prediction_dir", default='greedy_eval', type=str, help='for prediction')
    ...
    if args.do_eval:
        ...
        if args.beam == 1:
            ...
            args.output_dir = args.output_dir + '_beam_eval'
    else:
    ...
    if os.path.exists(args.output_dir) and os.listdir(
            args.output_dir) and (args.do_train or args.continue_train) and not args.overwrite_output_dir:
        raise ValueError(
    ...
    if args.do_train:
        ...
        model = model_class(config=config,
                            ...
                            ex_preseqlen=args.ex_preseqlen,
                            task_preseqlen=args.task_preseqlen,
                            mid_dim=args.preseqdim,)

        model.transformer = model.transformer.from_pretrained(load_from_path,
                                                            source_length=args.source_length,)
    else:
    ...
    tokenizer = add_custom_tokens(tokenizer)  # 添加一些GPT2字典没有的特殊tokens
    ...
    if args.do_train or args.continue_train:
        ...
        for name, param in model.named_parameters():
            ...
            if args.fine_tune_step != -1:  # all params will be updated
                if args.fine_tune_step == 0:
                    if 'transformer' in name:  # gpt2  prefix tuning on atomic
                        param.requires_grad = False
                    else:  # prefix - 1
                        param.requires_grad = True
                        new_params.append(name)
                elif args.fine_tune_step == 1:  # first prefix tuning on atomic, then prefix tuning on task data
                    if 'transformer' in name or 'task' not in name:  # gpt or prefix - 1
                        param.requires_grad = False
                    else:  # prefix -2
                        param.requires_grad = True
                        new_params.append(name)
                # elif args.fine_tune_step == 2: # prefix tuning on task data
                else:
                    print('en heng ?')
            else:
                break
