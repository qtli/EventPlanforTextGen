...
from data import ed_DatasetHelper
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, ori_batch in enumerate(train_dataloader):
            ...
            batch += [args.copy, args.embed_or_hidden, args.temperature]
            model.train()
            outputs = model(*batch)
            loss = outputs
            loss = loss.mean()
...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0'):
    ...
    eval_loss = 0.0
    tgen_seqs, multi_tgen_seqs = [], []
    src, tgt, src_event, tgt_event, src_kw, tgt_kw = [], [], [], [], [], []
    nb_eval_steps = 0
    model.eval()
    ...
    for bi, batch in enumerate(eval_dataloader):
        if args.do_eval or 'bleu' in evaluate_metrics:
            src.extend(batch[1])
            tgt.extend(batch[2])
            src_event.extend(batch[3])
            tgt_event.extend(batch[4])
            src_kw.extend(batch[5])
            tgt_kw.extend(batch[6])
        ...
        with torch.no_grad():
            if 'bleu' in evaluate_metrics:  # when inference
                model_batch = {"src_input_ids": batch[0],
                               ...
                               "event_input_ids": batch[6],
                               "event_attention_mask": batch[7],
                               "event_position_ids": batch[8],
                               "kw_input_ids": batch[9],
                               "kw_position_ids": batch[10],
                               "kw_attention_mask": batch[11],
            ...
            if 'ppl' in evaluate_metrics:
                eval_batch = [batch[0], batch[1], batch[2],
                         ...
                         batch[6], batch[7], batch[8],
                         batch[9], batch[10], batch[11], ]
                eval_batch += [args.copy, args.embed_or_hidden, args.temperature]
...
def save_generation(args, src, tgt, prediction,
                    src_event, tgt_event, src_kw, tgt_kw, prefix='0', multi_prediction=None):
    ...
    with open(save_result_dir, 'w') as f:
        for i, line in enumerate(src):
            ...
            f.write('src_event: ' + src_event[i] + '\n')
            f.write('tgt_event: ' + tgt_event[i] + '\n')
            f.write('src_kw: ' + src_kw[i].strip() + '\n')
            f.write('tgt_kw: ' + tgt_kw[i] + '\n')
