...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0'):
    ...
    if 'bleu' in evaluate_metrics:
        print_data = {'src': src, 'tgt': tgt, 'prediction': gen_seqs, 'prefix': prefix}
        if args.beam > 1:
...
def main():
    ...
    parser.add_argument("--ex_preseqlen", default=5, type=int, help="prefix length for prefix tuning")
    parser.add_argument("--task_preseqlen", default=5, type=int, help="prefix length for prefix tuning")
    ...
    if args.do_train or args.continue_train:
        ...
        #     else:
        #         if 'transformer' in name or 'task' not in name:
        #             param.requires_grad = False
        #         else:
        #             param.requires_grad = True
        #             new_params.append(name)
